...
async function handleCheckoutCompleted(event: Stripe.Event): Promise<StripeEventResult> {
...
async function handlePaymentSucceeded(event: Stripe.Event): Promise<StripeEventResult> {
...
async function handlePaymentFailed(event: Stripe.Event): Promise<StripeEventResult> {
  const paymentIntent = event.data.object as Stripe.PaymentIntent;
  ...
  try {
    ...
    if (donation && donation.status === "INIT") {
      ...
      return { processed: true, donationId: donation.id, action: "payment_failed" };
    }
