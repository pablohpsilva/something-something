...
function shouldProcessEvent(event: EnrichedEvent, ipHash: string): boolean {
  ...
  if (
    event.type === "VIEW" &&
    event.ruleId &&
    shouldDedupeView(ipHash, event.ruleId)
  ) {
    return false;
  }
...
export async function recordEvents(
    ...
    if (burstData && now.getTime() - burstData.firstSeen < burstWindowMs) {
      ...
      if (burstData.count > maxEventsPerMin) {
        ...
        const allowedCount = Math.max(
          0,
          maxEventsPerMin - (burstData.count - groupEvents.length)
        );
        ...
        logger.warn("Burst protection triggered", {
          ...
          ipHash: eventIpHash?.substring(0, 8) || "unknown",
  ...
  if (filteredEvents.length > 0) {
    try {
      ...
      if (anomalyEvents.length > 0 && anomalyEvents[0]) {
