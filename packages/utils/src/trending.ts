...
export function calculateTrendingScore(metrics: TrendingMetrics): number {
  ...
  const now = new Date()
  const ageInHours = Math.max(0, (now.getTime() - metrics.createdAt.getTime()) / (1000 * 60 * 60))
 ...
 */
export function getTrendingCategory(score: number): 'hot' | 'trending' | 'rising' | 'normal' {
